...
int test_crypto_hash_sha512()
{
  ...
  r = crypto_hash_sha512(out, in, in_len);
...
int test_crypto_auth_hmacsha256()
{
  ...
  r = crypto_auth_hmacsha256(out, in, in_len,key);
  print_uca(out,crypto_auth_hmacsha256_BYTES);

  // r should be 0  
  r |= crypto_auth_hmacsha256_verify(out, in, in_len, key);
  if(r != 0) return r;
...
)
{
  ...
  r = crypto_secretbox_easy(out, in, in_len, nonce, key);
  print_uca(out,crypto_secretbox_MACBYTES+LEN);

  // r should be 0  
  r |= crypto_secretbox_open_easy(in_d, out, crypto_secretbox_MACBYTES+LEN, nonce, key);
  if(r != 0) return r;

  // in_d == in
  r |= sodium_memcmp(in, in_d, LEN);
  if(r != 0) return r;
...
)
{
  ...
  r = crypto_secretbox(out, in, in_len, nonce, key);
  print_uca(out,out_len);

  // r should be 0  
  r |= crypto_secretbox_open(in_d, out, out_len, nonce, key);
  if(r != 0) return r;
...
int test_crypto_sign()
{
  ...
  r |= crypto_sign(signed_message, &signed_message_len,
                   message, message_len,
                   sk);
  print_uca(signed_message,signed_message_len);
  if(r != 0) return r;

  r |= crypto_sign_open(recovered_message,&recovered_message_len,
...
int test_crypto_verify()
{
  ...
  r = crypto_verify_32(a,b);
  r |= crypto_verify_16(a,b);
  print_uca(a,32);
  print_uca(b,32);
  if(r != 0) return r;
