...
// This Class is associated to the first version of multi-map encryption scheme (in the OXT paper) where the BF ids are stored in the EMM itself. This
// class needs to be
...
public class IEXZMF implements Serializable {
	...
	public static void setup(ObjectOutputStream output, List<byte[]> listOfkeys, String pwd, int maxLengthOfMask, int falsePosRate) throws Exception {

		TextProc.process(false, pwd);
		...
		constructMatryoshkaPar(new ArrayList(TextExtractPar.lp1.keySet()), listOfkeys.get(0), listOfkeys.get(1), maxLengthOfMask, falsePosRate);
	...
	public static List<Token> token(List<byte[]> listOfkeys, List<String> search, int falsePosRate, int maxLengthOfMask) throws Exception {
	...
	public static List<String> query(List<Token> token, IEX2Lev disj, int bucketSize, int falsePosRate) throws InvalidKeyException, InvalidAlgorithmParameterException, NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException, IOException {
		List<String> result = new ArrayList<String>();

		for (int i = 0; i < token.size(); i++) {
			List<String> resultTMP = RR2Lev.query(token.get(i).getTokenMMGlobal(), disj.getGlobalMM().getDictionary(), disj.getGlobalMM().getArray());
			...
			if ((i < token.size() - 1) && !(bFIDPaddeds == null)) {
				...
				for (int j = 0; j < bFIDPaddeds.size(); j++) {
					...
					listOfbloomFilter.put(bloomFilterID.get(bFID), bloomFilterMap.get(Integer.toString(bFID)).getSecureSetM());
	...
	public static void constructMatryoshkaPar(List<String> listOfKeyword, final byte[] keySM, final byte[] keyInvInd, final int maxLengthOfMask, final int falsePosRate) throws InterruptedException, ExecutionException, IOException {
			...
			service.execute(new Runnable() {
				...
				public void run() {
					...
					while (concurrentMap.keySet().size() > 0) {
						...
						try {
							bloomFilterList.addAll(secureSetMPar(input, keySM, keyInvInd, maxLengthOfMask, falsePosRate));
						} catch (InvalidKeyException | NoSuchAlgorithmException | NoSuchProviderException | NoSuchPaddingException | IOException e) {
	...
	public static List<ZMFFormat> secureSetMPar(String[] input, byte[] keySM, byte[] keyInvInd, int maxLengthOfMask, int falsePosRate) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchProviderException, NoSuchPaddingException, IOException {
		...
		for (String keyword : input) {
			...
			System.out
					.println("\n \n Number of keywords processed % " + (numberOfkeywordsProcessed * 100) / TextExtractPar.lp1.keySet().size() + "\n");

			System.out.println("keyword being processed to issue matryoshka filters: " + keyword);

			Map<String, boolean[]> secureSetM2 = ZMF.setupSetMV2(keySM, keyword, TextExtractPar.lp2, TextExtractPar.lp1, falsePosRate);
			int counter = 0;
			for (String id : secureSetM2.keySet()) {
				result.add(new ZMFFormat(secureSetM2.get(id), Integer.toString(numberOfBF)));

				bloomFilterMap.put(Integer.toString(numberOfBF), new ZMFFormat(secureSetM2.get(id), Integer.toString(numberOfBF)));
				if (counter == 0) {
